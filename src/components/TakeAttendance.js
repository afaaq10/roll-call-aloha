...
const TakeAttendance = () => {
    const [students, setStudents] = useState([]);
    const { selectedProgram } = useParams();  // Use useParams to extract selectedProgram from URL

    useEffect(() => {
        console.log("Selected Program:", selectedProgram); // Check if selectedProgram is available
        const getStudents = async () => {
    ...
    const markAttendance = async (studentId, status) => {
        ...
        const updatedStudentsData = await fetchStudents(selectedProgram); // Re-fetch after marking attendance
        setStudents(updatedStudentsData);
    };
